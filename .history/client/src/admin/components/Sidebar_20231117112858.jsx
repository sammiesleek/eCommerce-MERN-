    ...
    <div className="fixed w-screen h-screen flex ">
      ...
      <div className="flex w-[300px] flex-col border-r bg-white">
        ...
        <div className="flex flex-col w-full h-full pt-4">
          <NavLink
            to="/admin"
            className="flex pl-4 no-underline gap-x-3 py-3 text-[#555962] font-semibold "
          >
            <ApplicationMenu
              theme="outline"
              size="28"
          ...
          <MetisMenu toggle={false} className="pl-0">
            {content.map((item, index) => (
              <li key={index}>
                <NavLink className="flex  no-underline gap-x-3 py-2 pl-4  text-[#555962] font-semibold ">
                  {item.icon}
                ...
                <ul>
                  {item.content &&
                    item.content.map((item, index) => (
                      <li key={index}>
                        <NavLink
                          className="no-underline  text-[#555962] font-normal hover:tex-[#10B981] transition-all ease-linear"
                          to="/about"
                        >
                          {item.label}
                        </NavLink>
