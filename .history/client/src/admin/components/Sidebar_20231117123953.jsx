...
const SideBar = () => {
  ...
  return (
    <div className="fixed w-screen h-screen flex ">
      <div className="hidden lg:flex w-screen bg-red-400 absolute inset-0 bg-transparent" />
      <div className="flex w-[200px] flex-col border-r bg-white relative z-20">
        ...
        >
          <span className="font-bold text-[#10B981] text-3xl">WizBee</span>
        </Link>
          ...
          <MetisMenu toggle={false} className="pl-0">
            {content.map((item, index) => (
              <li key={index}>
                <NavLink
                  to=""
                  className="flex  no-underline gap-x-3 py-2 pl-4  text-[#555962] font-semibold hover:text-[#10B981] transition-all ease-linear"
                >
                  {item.icon}
